...
function renderDateTime() {
  ...
  const el = $('dateTime');
  if (!el) return;
  // Actualiza solo el texto, sin reconstruir HTML (evita reflow en cada segundo)
  const dayEl = el.querySelector('.day');
  const timeEl = el.querySelector('.hud-time');
  if (dayEl && dayEl.textContent !== dayStr.charAt(0).toUpperCase()+dayStr.slice(1)) {
    dayEl.textContent = dayStr.charAt(0).toUpperCase()+dayStr.slice(1);
  }
  if (timeEl && timeEl.textContent.split('·')[0].trim() !== timeStr) {
    timeEl.textContent = `${timeStr} · La Paz, BCS · UTC-7`;
  }
}
...
function getKanban() {
  // Versión personal: el kanban es ESPEJO de Nexus (fuente de verdad)
  if (!CFG.publicMode && window.__nexusKanban) return window.__nexusKanban;
...
function addKanbanCard(col) {
  // Versión personal: las tareas viven en Nexus — se crean allá
  if (!CFG.publicMode) {
    window.open('https://nexus.oscaromargp.xyz', '_blank');
    return;
  }
...
async function syncGithubData(btn) {
  ...
  try {
    if (CFG.publicMode) {
      ...
      if (success) { renderKanban(); renderPriorities(); renderDiaTab(); }
      await window.GithubSync.syncUp();
    } else {
      // Versión personal: recarga las tareas REALES de Nexus
      await loadNexusData();
      renderKanban();
      renderPriorities();
      renderDiaTab();
...
async function loadNexusData() {
  try {
    ...
    if (data && data.kanban) {
      window.__nexusKanban = data.kanban;
      if (data.resumen) window.__nexusResumen = data.resumen;
      renderNexusResumen();
    }
    return true;
  } catch { return false; }
...
function renderNexusResumen() {
  const r = window.__nexusResumen;
  if (!r) return;
  const tldr = $('tldrText');
  if (tldr && (r.critico && r.critico.length)) {
    const base = tldr.textContent.split(' — ')[0];
    tldr.textContent = `${base} — 🔴 Crítico hoy: ${r.critico.slice(0,3).map(t=>t.text).join(' · ')}`;
  }
  const pc = $('planContent');
  if (pc && !pc.dataset.nexus && (r.critico.length || r.venceHoy.length)) {
    pc.dataset.nexus = '1';
    const extra = `
      <div class="card" style="margin-bottom:10px;border-color:rgba(239,68,68,.4);">
        <div style="font-size:.75rem;letter-spacing:.08em;color:#f87171;font-weight:600;margin-bottom:6px;">🔴 CRÍTICO HOY (Nexus)</div>
        <ul style="padding-left:18px;font-size:.84rem;line-height:1.7;color:var(--text-secondary);">
          ${r.critico.slice(0,5).map(t=>`<li>${esc(t.text)}</li>`).join('')}
        </ul>
        ${r.venceHoy.length ? `<div style="font-size:.75rem;letter-spacing:.08em;color:#fbbf24;font-weight:600;margin:8px 0 4px;">⏰ VENCE HOY</div>
        <ul style="padding-left:18px;font-size:.84rem;line-height:1.7;color:var(--text-secondary);">
          ${r.venceHoy.slice(0,5).map(t=>`<li>${esc(t.text)}${t.amount?` · <span style="color:#fbbf24">$${t.amount}</span>`:''}</li>`).join('')}
        </ul>` : ''}
      </div>`;
    pc.insertAdjacentHTML('afterbegin', extra);
  }
}

// Override saveKanban: en modo público empuja a GitHub (debounced); en personal no-op (Nexus es la fuente)
let syncTimeout;
const _origSaveKanban = saveKanban;
window.saveKanban = function(data) {
  _origSaveKanban.call(this, data);
  if (CFG.publicMode && window.GithubSync) {
    clearTimeout(syncTimeout);
    syncTimeout = setTimeout(() => {
      try { window.GithubSync.syncUp(); } catch {}
    }, 5000);
  }
};
...
async function fetchWeather() {
  ...
  const results = await Promise.allSettled(
    cities.map(c => cachedFetch('boletin_clima_' + c.name, 10*60*1000,
      () => fetch(`${CFG.openMeteo}?latitude=${c.lat}&longitude=${c.lon}&${params}&timezone=${c.tz}`).then(r=>r.json())
    ))
  );
...
async function fetchCripto() {
  ...
  try {
    // Cache persistente de 5 min (evita re-fetch en cada render)
    const priceData = await cachedFetch('boletin_cripto', 5*60*1000, () => fetch(url).then(r=>r.json()));
    cc.innerHTML = `
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  renderKanban();
  renderPriorities();
  // Paneles personales solo en la versión personal (VPS)
  if (!CFG.publicMode) {
    renderSalud();
    renderHijo();
    loadNexusData().then(() => { renderKanban(); renderPriorities(); });
  } else {
    // Demo pública: secciones personales ocultas
    ['s-salud','s-hijo'].forEach(id => { const el = document.getElementById(id); if (el) el.style.display = 'none'; });
  }
  renderMusica();
  renderProductividad();
  renderProyectos();
